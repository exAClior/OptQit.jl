...
target1(As,Bs,ρ,score::Function) = real(sum([score(a, b, x, y) * tr(kron(As[(a) * 2 + x + 1], Bs[(b) * 2 + y + 1]) * ρ) for a in 0:1, b in 0:1, x in 0:1, y in 0:1]))

function chsh_op_state(target::Function,As::AbstractVector{MT},Bs::AbstractVector{MT},restriction::Expr,nlevels::Pair{Int,Int}; optimizer= Mosek.Optimizer, silent=true) where {MT<:AbstractMatrix}
    ...
    error("Failed to find the optimal State.")
end


function chsh_op_povm(target::String, As, Bs, ρ, nlevels::Pair{Int,Int}; optimizer = Mosek.Optimizer, silent = true)
...
@testset "entanglement" begin
	...
	ρ_init = [0.3943 0.0348 0.0348 0.3282;
